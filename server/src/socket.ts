...
export class Socket extends EventEmitter {
  ...
  private timeSyncClockDeltaQueue: {
    clockDelta: number;
  ...
  clientTimeToServerTime(timeOfMovementInClientTime: number) {
    const timeOfMovementInServerTime =
      timeOfMovementInClientTime + this.timeSyncClockDelta;
    if (
      this.timeSyncClockDelta === 0 ||
      timeOfMovementInServerTime < 0 ||
      timeOfMovementInServerTime > 0xffffffff
    ) {
      console.error(
  ...
  handleTimeSyncResponseOpcode(sequenceIndex: number, clientTime: number) {
    if (!this.pendingTimeSyncRequests.has(sequenceIndex)) return;
    ...
    this.timeSyncClockDeltaQueue[(this.timeSyncNextCounter - 1) % 6] = {
      clockDelta,
      roundTripDuration,
    };
    ...
    const latencyAccumulator = this.timeSyncClockDeltaQueue.map(
      ({ roundTripDuration }) => roundTripDuration,
    );
    latencyAccumulator.sort((a, b) => a - b);

    const latencyMedian =
      latencyAccumulator[Math.floor(latencyAccumulator.length / 2)];
    const latencyStandardDeviation = Math.sqrt(
      latencyAccumulator.reduce(
        (acc, latency) => acc + (latency - latencyMedian) ** 2,
        0,
      ) / latencyAccumulator.length,
    );

    const filteredClockDeltaQueue = this.timeSyncClockDeltaQueue.filter(
      ({ roundTripDuration }) =>
        roundTripDuration < latencyMedian + latencyStandardDeviation,
    );

    if (filteredClockDeltaQueue.length) {
      const meanClockDelta =
        filteredClockDeltaQueue.reduce(
          (acc, { clockDelta }) => acc + clockDelta,
          0,
        ) / filteredClockDeltaQueue.length;
      this.timeSyncClockDelta = meanClockDelta;
    } else {
      this.timeSyncClockDelta =
        this.timeSyncClockDeltaQueue[
          this.timeSyncClockDeltaQueue.length - 1
        ].clockDelta;
    }
  }
}
